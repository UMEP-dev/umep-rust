...
def gvf_2018a(
    ...
    for j in np.arange(0, azimuthA.__len__()):
        _, gvfLupi, gvfalbi, gvfalbnoshi, gvf2 = sun.sun_on_surface(
            azimuthA[j],
            scale,
            buildings.astype(np.float32),
            shadow.astype(np.float32),
            sunwall.astype(np.float32),
            first,
            second,
            (dirwalls * np.pi / 180).astype(np.float32),
            walls.astype(np.float32),
            Tg.astype(np.float32),
            Tgwall.astype(np.float32),
            Ta,
