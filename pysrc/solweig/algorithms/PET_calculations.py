...
try:
    from ..rustalgos import pet as _rust_pet
...
def calculate_PET_grid_fast(Ta, RH, Tmrt, va, pet):
    """
    Fast PET grid calculation using Rust when available.
    ...
    Parameters:
        ...
        va: Wind speed grid (m/s) - 2D array
        pet: PET_person namedtuple with person parameters

    Returns:
        PET temperature grid (°C)
    """
    ...
    if _RUST_AVAILABLE:
        ...
        return _rust_pet.pet_grid(
            float(Ta), float(RH), Tmrt, va,
            float(pet.mbody), float(pet.age), float(pet.height),
            float(pet.activity), float(pet.clo), int(pet.sex)
        )
    else:
        # Fallback to Python (slow but works)
        pet_index = np.zeros_like(Tmrt, dtype=np.float32)
        for y in range(pet_index.shape[0]):
            for x in range(pet_index.shape[1]):
                if va[y, x] > 0:
                    pet_index[y, x] = _PET(
                        Ta, RH, Tmrt[y, x], va[y, x],
                        pet.mbody, pet.age, pet.height,
                        pet.activity, pet.clo, pet.sex
                    )
                else:
                    pet_index[y, x] = -9999
        return pet_index
