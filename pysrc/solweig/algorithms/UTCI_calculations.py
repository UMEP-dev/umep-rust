...
# Try to import Rust implementation for faster grid calculations
try:
    from ..rustalgos import utci as _rust_utci
...
def utci_calculator_grid(Ta, RH, Tmrt, va10m, feedback):
    ...
    if Ta <= -999 or RH <= -999:
        UTCI_approx = np.ones((rows, cols), dtype=np.float32) * -999
    # Else, create an empty raster for UTCI and start estimating UTCI
    else:
        UTCI_approx = np.zeros((rows, cols), dtype=np.float32)
...
def utci_calculator_grid_fast(Ta, RH, Tmrt, va10m):
    """
    Fast UTCI grid calculation using Rust when available.

    This is a simplified version without feedback/progress for use in batch processing.
    Falls back to vectorized NumPy if Rust is not available.

    Parameters:
        Ta: Air temperature (°C) - scalar
        RH: Relative humidity (%) - scalar
        Tmrt: Mean radiant temperature grid (°C) - 2D array
        va10m: Wind speed grid at 10m (m/s) - 2D array

    Returns:
        UTCI temperature grid (°C)
    """
    # Ensure arrays are float32 and contiguous
    Tmrt = np.ascontiguousarray(Tmrt, dtype=np.float32)
    va10m = np.ascontiguousarray(va10m, dtype=np.float32)

    if _RUST_AVAILABLE:
        # Use Rust parallel implementation
        return _rust_utci.utci_grid(float(Ta), float(RH), Tmrt, va10m)
    else:
        # Fallback to vectorized NumPy (faster than the nested loop version)
        rows, cols = Tmrt.shape

        if Ta <= -999 or RH <= -999:
            return np.ones((rows, cols), dtype=np.float32) * -999
        ...
        valid = (Tmrt > -999) & (va10m > 0)

        result = np.full((rows, cols), -9999.0, dtype=np.float32)

        # Calculate UTCI only for valid pixels (still uses the polynomial per-pixel)
        for iy in range(rows):
            for ix in range(cols):
                if valid[iy, ix]:
                    result[iy, ix] = utci_polynomial(D_Tmrt[iy, ix], Ta, va10m[iy, ix], Pa)
