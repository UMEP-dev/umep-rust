...
def load_walls(voxelTable, solweig_parameters, wall_type, wallaspect, Ta, timeStep, albedo_grid, landcover, lcgrid, dsm):
    ...
    # Calculate fractions
    # voxelTable.loc[voxelTable['svfbu'] > 0.5, 'svfbu'] = 0.5
    # voxelTable.loc[voxelTable['svfaveg'] > 0.5, 'svfaveg'] = 0.5
    ...
    eqTime = True
    if gvc_sim:
        # Changed aspect on GVC walls
        print('wall aspect for wall 919 before change:')
        print(voxelTable.loc[voxelTable['wallId'] == 919, 'wallAspect'])
        print('wall aspect for wall 925 before change:')
        print(voxelTable.loc[voxelTable['wallId'] == 925, 'wallAspect'])

        east_angle = 130
        west_angle = 210
        # print('wall aspect for wall 919 after change:')
        voxelTable.loc[voxelTable['wallId'] == 914, 'wallAspect'] = east_angle # 914 = east
        voxelTable.loc[voxelTable['wallId'] == 920, 'wallAspect'] = east_angle # 920 = southeast
        voxelTable.loc[voxelTable['wallId'] == 926, 'wallAspect'] = east_angle # 926 = south/southeast
        voxelTable.loc[voxelTable['wallId'] == 919, 'wallAspect'] = west_angle # 919 = west        
        voxelTable.loc[voxelTable['wallId'] == 925, 'wallAspect'] = west_angle # 925 = southwest
        
        print('wall aspect for wall 920 after change:')
        print(voxelTable.loc[voxelTable['wallId'] == 920, 'wallAspect'])
        print('wall aspect for wall 925 after change:')
        print(voxelTable.loc[voxelTable['wallId'] == 925, 'wallAspect'])
        wall_id = np.array([914, 919, 920, 925, 926])
        aspect_dict = {914: 135, 919: 220, 920: 135, 925: 220, 926: 135}
        for temp_id in wall_id:
            temp_y = voxelTable.loc[voxelTable['wallId'] == temp_id, 'ypos'].to_numpy().astype(int)
            temp_x = voxelTable.loc[voxelTable['wallId'] == temp_id, 'xpos'].to_numpy().astype(int)
            wallaspect[temp_y, temp_x] = aspect_dict[temp_id]

    elif kolumbus_sim == True:
        voxelTable.loc[voxelTable['voxelHeight'] >= 4, 'thermalEffusivity'] = 472
        voxelTable.loc[voxelTable['voxelHeight'] >= 4, 'wallAlbedo'] = 0.6
        voxelTable.loc[voxelTable['voxelHeight'] >= 4, 'wallEmissivity'] = 0.95
        voxelTable.loc[voxelTable['voxelHeight'] >= 4, 'timeStep'] = 0.03**2/(np.pi**2*0.13*10**-6)

        voxelTable.loc[voxelTable['voxelHeight'] < 4, 'thermalEffusivity'] = (1068+860)/2
        voxelTable.loc[voxelTable['voxelHeight'] < 4, 'wallAlbedo'] = 0.5
        voxelTable.loc[voxelTable['voxelHeight'] < 4, 'wallEmissivity'] = 0.95
        voxelTable.loc[voxelTable['voxelHeight'] < 4, 'timeStep'] = 0.1**2/(np.pi**2*((0.62+0.44)/2)*10**-6)
        # voxelTable['thermalEffusivity'] = 472
        # voxelTable['wallAlbedo'] = 0.52
        # voxelTable['wallEmissivity'] = 0.95
        # voxelTable['timeStep'] = 0.03**2/(np.pi**2*0.13*10**-6)

        # voxelTable['thermalEffusivity'] = (1068+860)/2
        # voxelTable['wallAlbedo'] = 0.52
        # voxelTable['wallEmissivity'] = 0.95
        # voxelTable['timeStep'] = 0.1**2/(np.pi**2*((0.62+0.44)/2)*10**-6)        
