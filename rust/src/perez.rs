...
const M_A1: [f32; 8] = [1.3525, -1.2219, -1.1000, -0.5484, -0.6000, -1.0156, -1.0000, -1.0500];
const M_A2: [f32; 8] = [-0.2576, -0.7730, -0.2515, -0.6654, -0.3566, -0.3670, 0.0211, 0.0289];
const M_A3: [f32; 8] = [-0.2690, 1.4148, 0.8952, -0.2672, -2.5000, 1.0078, 0.5025, 0.4260];
const M_A4: [f32; 8] = [-1.4366, 1.1016, 0.0156, 0.7117, 2.3250, 1.4051, -0.5119, 0.3590];

const M_B1: [f32; 8] = [-0.7670, -0.2054, 0.2782, 0.7234, 0.2937, 0.2875, -0.3000, -0.3250];
const M_B2: [f32; 8] = [0.0007, 0.0367, -0.1812, -0.6219, 0.0496, -0.5328, 0.1922, 0.1156];
const M_B3: [f32; 8] = [1.2734, -3.9128, -4.5000, -5.6812, -5.6812, -3.8500, 0.7023, 0.7781];
const M_B4: [f32; 8] = [-0.1233, 0.9156, 1.1766, 2.6297, 1.8415, 3.3750, -1.6317, 0.0025];

const M_C1: [f32; 8] = [2.8000, 6.9750, 24.7219, 33.3389, 21.0000, 14.0000, 19.0000, 31.0625];
const M_C2: [f32; 8] = [0.6004, 0.1774, -13.0812, -18.3000, -4.7656, -0.9999, -5.0000, -14.5000];
const M_C3: [f32; 8] = [1.2375, 6.4477, -37.7000, -62.2500, -21.5906, -7.1406, 1.2438, -46.1148];
const M_C4: [f32; 8] = [1.0000, -0.1239, 34.8438, 52.0781, 7.2492, 7.5469, -1.9094, 55.3750];

const M_D1: [f32; 8] = [1.8734, -1.5798, -5.0000, -3.5000, -3.5000, -3.4000, -4.0000, -7.2312];
const M_D2: [f32; 8] = [0.6297, -0.5081, 1.5218, 0.0016, -0.1554, -0.1078, 0.0250, 0.4050];
const M_D3: [f32; 8] = [0.9738, -1.7812, 3.9229, 1.1477, 1.4062, -1.0750, 0.3844, 13.3500];
const M_D4: [f32; 8] = [0.2809, 0.1080, -2.6204, 0.1062, 0.3988, 1.5702, 0.2656, 0.6234];

const M_E1: [f32; 8] = [0.0356, 0.2624, -0.0156, 0.4659, 0.0032, -0.0672, 1.0468, 1.5000];
const M_E2: [f32; 8] = [-0.1246, 0.0672, 0.1597, -0.3296, 0.0766, 0.4016, -0.3788, -0.6426];
const M_E3: [f32; 8] = [-0.5718, -0.2190, 0.4199, -0.0876, -0.0656, 0.3017, -2.4517, 1.8564];
const M_E4: [f32; 8] = [0.9938, -0.4285, -0.5562, -0.0329, -0.1294, -0.4844, 1.4656, 0.5636];
...
pub(crate) fn create_patches(patch_option: i32) -> (Vec<f32>, Vec<f32>) {
    let (skyvault_alt_int, azistart, patches_in_band): (&[f32], &[f32], &[i32]) =
        match patch_option {
            1 => (
...
pub(crate) fn compute_steradians(altitudes: &[f32]) -> Array1<f32> {
    ...
    for i in 0..n {
        ...
        let count = counts[unique_alts.iter().position(|&u| (u - alt_i).abs() < 1e-6).unwrap()];
        if count > 1 {
            steradian[i] = (360.0 / count as f32) * DEG2RAD
                * (((alt_i + first_alt) * DEG2RAD).sin()
                    - ((alt_i - first_alt) * DEG2RAD).sin());
        } else {
            ...
            steradian[i] = (360.0 / count as f32) * DEG2RAD
                * ((alt_i * DEG2RAD).sin()
                    - ((prev_alt + first_alt) * DEG2RAD).sin());
        }
    }
...
pub(crate) fn perez_v3(
    ...
    let (altitudes, azimuths) = create_patches(patch_option);
    let n = altitudes.len();
    ...
    for i in 0..n {
        ...
        let cos_sky_sun = sv_alt.sin() * sin_alt
            + cos_alt * sv_alt.cos() * (sv_azi - azimuth).abs().cos();
...
mod tests {
    ...
    fn test_perez_normalised() {
        ...
        assert!((sum - 1.0).abs() < 1e-5, "luminance sum = {sum}, expected 1.0");
    }
