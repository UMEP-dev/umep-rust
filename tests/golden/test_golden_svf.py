...
Vegetation SVF has a known ~17% max difference (0.17% of pixels) because the
golden fixtures are generated by UMEP's svfForProcessing153, which internally
uses shadowingfunction_20, while our Rust SVF uses the wallheight_23-equivalent
shadow function. Both implement the same pergola heuristic, but floating-point
accumulation across 153 patches diverges at vegetation edges.

TODO: Regenerate SVF golden fixtures using shadowingfunction_wallheight_23
(the same function used for shadow goldens) for a like-for-like comparison.
"""
...
# Vegetation SVF tolerance is wider because the golden fixture was generated
# by UMEP's svfForProcessing153 (which uses shadowingfunction_20), while our
# Rust SVF uses the wallheight_23-equivalent shadow function. Both implement
# the same pergola heuristic but float accumulation across 153 patches diverges
# at vegetation edges (0.11% of pixels, max abs diff ~0.17).
# TODO: Regenerate fixture from shadowingfunction_wallheight_23 for parity.
VEG_RTOL = 0.21
VEG_ATOL = 0.18
